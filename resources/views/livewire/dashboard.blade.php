<div class="min-h-screen bg-slate-950"
     x-data="{
        ...
        async confirmDelete() {
            ...
            if (this.isBulkDelete) {
                await $wire.bulkDelete(this.selectedItems);
                this.selectedItems = [];
                this.selectMode = false;
            } else {
                await $wire.deleteItem(this.deleteTargetId);
            }
            this.closeModal();
        },
        exitSelectMode() {
     ...
     x-on:bulk-delete-done.window="selectedItems = []; selectMode = false;"
     x-on:bulk-action-done.window="selectedItems = []; selectMode = false;"
     x-on:open-url.window="window.open($event.detail[0].url, '_blank')"
>
    {{-- Main Container --}}
        ...
        <nav class="md:hidden fixed bottom-0 left-0 w-full z-50 pointer-events-none pb-4 sm:pb-6 px-4 sm:px-8">
            <div class="w-full h-[64px] bg-slate-900/95 backdrop-blur-2xl rounded-2xl flex items-center relative border border-slate-800/50 shadow-2xl shadow-black pointer-events-auto" x-data="{ active: {{ request()->routeIs('admin.dashboard') ? 0 : (request()->routeIs('admin.students') ? 2 : 0) }} }">
                <div class="absolute -top-[24px] w-[56px] h-[56px] bg-primary-600 rounded-full transition-all duration-500 ease-[cubic-bezier(0.68,-0.55,0.265,1.55)] shadow-[0_8px_16px_rgba(14,165,233,0.3)] pointer-events-none z-0"
                     :style="`left: calc((${active} * 33.333%) + 16.666% - 28px)`"></div>
                <ul class="flex w-full h-full relative z-10 bg-transparent">
                    <li class="flex-1 flex w-full">
                        <a href="{{ route('admin.dashboard') }}" wire:navigate class="w-full h-full flex flex-col items-center justify-center relative group" @click="active = 0">
                            <span class="absolute flex items-center justify-center w-full h-full transition-all duration-500 ease-[cubic-bezier(0.68,-0.55,0.265,1.55)] z-20" :class="active === 0 ? '-translate-y-[24px] text-white' : 'text-slate-400 group-hover:text-primary-300'">
                                <svg class="w-[22px] h-[22px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                            </span>
                            <span class="absolute bottom-1.5 text-[9px] font-semibold tracking-wider transition-all duration-300" :class="active === 0 ? 'opacity-100 text-primary-400' : 'opacity-0 translate-y-4'">GALERI</span>
                        </a>
                    </li>
                    ...
                    <li class="flex-1 flex w-full">
                        <a href="{{ route('admin.students') }}" wire:navigate class="w-full h-full flex flex-col items-center justify-center relative group" @click="active = 2">
                            <span class="absolute flex items-center justify-center w-full h-full transition-all duration-500 ease-[cubic-bezier(0.68,-0.55,0.265,1.55)] z-20" :class="active === 2 ? '-translate-y-[24px] text-white' : 'text-slate-400 group-hover:text-primary-300'">
                                <svg class="w-[22px] h-[22px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                            </span>
                            <span class="absolute bottom-1.5 text-[9px] font-semibold tracking-wider transition-all duration-300" :class="active === 2 ? 'opacity-100 text-primary-400' : 'opacity-0 translate-y-4'">SISWA</span>
                        </a>
                    </li>
                </ul>
            </div>
...
<!-- Sidebar (Instagram style) -->
        <aside class="hidden md:flex fixed top-0 left-0 md:w-[80px] md:hover:w-[260px] md:h-screen md:border-t-0 md:border-r border-slate-800/50 flex flex-row md:flex-col z-50 bg-slate-950 md:bg-slate-950/90 md:backdrop-blur-2xl transition-[width] duration-300 ease-in-out group overflow-x-hidden overflow-y-auto custom-scrollbar">
            ...
            <div class="flex-1 w-full md:w-[260px] md:px-3 py-0 md:py-2 relative z-10 flex flex-row md:flex-col items-center md:items-start md:space-y-1 justify-evenly md:justify-start">
                ...
                <button wire:click="setAlbum('')" class="hidden md:flex items-center w-full p-2.5 rounded-xl transition-all duration-300 {{ $filterAlbum === '' ? 'text-primary-400 font-bold bg-primary-500/10' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white' }}">
                    <div class="w-[36px] flex items-center justify-center shrink-0"><svg class="w-[26px] h-[26px]" fill="none" stroke="currentColor" stroke-width="{{ $filterAlbum === '' ? '2.5' : '2' }}" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg></div>
                ...
                <button wire:click="setAlbum('Foto Profile')" class="hidden md:flex items-center w-full p-2.5 rounded-xl transition-all duration-300 {{ $filterAlbum === 'Foto Profile' ? 'text-primary-400 font-bold bg-primary-500/10' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white' }}">
                    <div class="w-[36px] flex items-center justify-center shrink-0"><svg class="w-[26px] h-[26px]" fill="none" stroke="currentColor" stroke-width="{{ $filterAlbum === 'Foto Profile' ? '2.5' : '2' }}" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></div>
                ...
                <button wire:click="setAlbum('Foto Grub')" class="hidden md:flex items-center w-full p-2.5 rounded-xl transition-all duration-300 {{ $filterAlbum === 'Foto Grub' ? 'text-primary-400 font-bold bg-primary-500/10' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white' }}">
                ...
                <button wire:click="setAlbum('Foto Lapangan')" class="hidden md:flex items-center w-full p-2.5 rounded-xl transition-all duration-300 {{ $filterAlbum === 'Foto Lapangan' ? 'text-primary-400 font-bold bg-primary-500/10' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white' }}">
                    <div class="w-[36px] flex items-center justify-center shrink-0"><svg class="w-[26px] h-[26px]" fill="none" stroke="currentColor" stroke-width="{{ $filterAlbum === 'Foto Lapangan' ? '2.5' : '2' }}" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></div>
                ...
                <button wire:click="setAlbum('Foto dan Vidio Random')" class="hidden md:flex items-center w-full p-2.5 rounded-xl transition-all duration-300 {{ $filterAlbum === 'Foto dan Vidio Random' ? 'text-primary-400 font-bold bg-primary-500/10' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white' }}">
            ...
            <div class="hidden md:flex flex-col gap-1 px-3 lg:pb-6 relative z-10 w-[260px] shrink-0">
                <a href="/" wire:navigate class="flex items-center w-full p-2.5 rounded-xl transition-all duration-300 text-slate-400 hover:bg-slate-800/50 hover:text-white">
                    <div class="w-[36px] flex items-center justify-center shrink-0"><svg class="w-[26px] h-[26px]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg></div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8 animate-fade-in">
            <div class="glass rounded-xl p-5 transition-all duration-300 hover:border-slate-700/50">
                <div class="flex items-center gap-4">
                    ...
                    <div><p class="text-2xl font-bold text-white">{{ $totalItems }}</p><p class="text-xs text-slate-400 uppercase tracking-wider">Total Item</p></div>
                </div>
            </div>
            <div class="glass rounded-xl p-5 transition-all duration-300 hover:border-slate-700/50">
                <div class="flex items-center gap-4">
                    ...
                    <div><p class="text-2xl font-bold text-white">{{ $totalPhotos }}</p><p class="text-xs text-slate-400 uppercase tracking-wider">Foto</p></div>
                </div>
            </div>
            <div class="glass rounded-xl p-5 transition-all duration-300 hover:border-slate-700/50">
                <div class="flex items-center gap-4">
                    ...
                    <div><p class="text-2xl font-bold text-white">{{ $totalVideos }}</p><p class="text-xs text-slate-400 uppercase tracking-wider">Video</p></div>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6 mt-8">
            <div class="relative flex-1 w-full">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari berdasarkan judul, deskripsi, atau kategori..." class="w-full pl-10 pr-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-xl text-sm text-white placeholder-slate-500 transition-all duration-300 focus:border-primary-500/50">
            </div>
            <div class="flex items-center gap-3 w-full sm:w-auto">
                ...
                {{-- Select Mode Toggle --}}
                <button
                    @click="selectMode = !selectMode; if(!selectMode) selectedItems = [];"
                    class="flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 border shrink-0"
                    :class="selectMode ? 'bg-primary-500/20 border-primary-500/40 text-primary-300' : 'bg-slate-800/50 border-slate-700/50 text-slate-400 hover:text-white hover:border-slate-600'"
                >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg>
                    <span class="hidden sm:inline" x-text="selectMode ? 'Batal Pilih' : 'Pilih'"></span>
                </button>
            </div>
        </div>

        {{-- Bulk Action Bar --}}
        <div x-show="selectMode && selectedItems.length > 0" x-transition class="mb-6">
            <div class="glass rounded-xl px-4 py-3 flex flex-wrap items-center justify-between gap-3 border border-primary-500/30">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg bg-primary-500/20 flex items-center justify-center">
                        <span class="text-sm font-bold text-primary-300" x-text="selectedItems.length"></span>
                    </div>
                    <span class="text-sm text-slate-300">item dipilih</span>
                </div>
                <div class="flex items-center gap-2">
                    <button @click="selectedItems = []" class="px-3 py-1.5 rounded-lg text-xs font-medium text-slate-300 bg-slate-800/50 border border-slate-700/50 hover:bg-slate-700 hover:text-white transition-all">
                        Batal Pilih
                    </button>
                    <button
                        wire:click="bulkDownload(selectedItems)"
                        class="px-4 py-1.5 rounded-lg text-xs font-semibold text-blue-300 bg-blue-500/15 border border-blue-500/30 hover:bg-blue-500/30 hover:text-blue-200 transition-all flex items-center gap-1.5"
                    >
                    ...
                    <button
                        ...
                        class="px-4 py-1.5 rounded-lg text-xs font-semibold text-red-300 bg-red-500/15 border border-red-500/30 hover:bg-red-500/30 hover:text-red-200 transition-all flex items-center gap-1.5"
                    >
        ...
        <div wire:loading.remove class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            @forelse($galleries as $item)
                <div class="gallery-item glass rounded-xl overflow-hidden group relative transition-all duration-200"
                     :class="isSelected({{ $item->id }}) ? 'ring-2 ring-primary-400 ring-offset-2 ring-offset-slate-950' : ''"
                    ...
                    <template x-if="selectMode">
                        <button
                            ...
                            class="absolute top-3 right-3 z-20 w-7 h-7 rounded-lg flex items-center justify-center transition-all duration-200 cursor-pointer"
                            :class="isSelected({{ $item->id }})
                                ? 'bg-primary-500 border-2 border-primary-400 shadow-lg shadow-primary-500/30'
                                : 'bg-black/40 backdrop-blur-sm border-2 border-white/30 hover:border-primary-400'"
                        >
                            <svg x-show="isSelected({{ $item->id }})" class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>
                        </button>
                    </template>
                    ...
                    <div class="relative aspect-[4/3] overflow-hidden bg-slate-800">
                        @if($item->isPhoto())
                            @if($item->isGoogleDrive())
                                <img src="{{ $item->getDisplayUrl() }}" alt="{{ $item->title }}" class="w-full h-full object-cover" loading="lazy" referrerpolicy="no-referrer">
                            @else
                                <img src="{{ asset('storage/' . $item->file_path) }}" alt="{{ $item->title }}" class="w-full h-full object-cover" loading="lazy">
                            @endif
                        @else
                            @if($item->isGoogleDrive())
                                <img src="{{ $item->getGoogleDriveThumbnailUrl() }}" alt="{{ $item->title }}" class="w-full h-full object-cover" loading="lazy" referrerpolicy="no-referrer">
                            @else
                                <video src="{{ asset('storage/' . $item->file_path) }}" class="w-full h-full object-cover" preload="metadata"></video>
                            @endif
                        ...
                        {{-- Source Badge --}}
                        @if($item->isGoogleDrive())
                            <div class="absolute top-3 right-3" x-show="!selectMode">
                        ...
                        {{-- Actions Overlay (visible on hover when NOT in select mode) --}}
                        <div x-show="!selectMode" class="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-end gap-2">
                            <button
                                wire:click="$dispatchTo('upload-form', 'editItem', { id: {{ $item->id }} })"
                                class="w-9 h-9 rounded-lg bg-white/10 backdrop-blur-sm border border-white/20 flex items-center justify-center text-white hover:bg-primary-500/50 transition-all"
                                title="Edit"
                            >
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </button>
                            <button
                                @click.stop="openSingleDelete({{ $item->id }})"
                                class="w-9 h-9 rounded-lg bg-white/10 backdrop-blur-sm border border-white/20 flex items-center justify-center text-white hover:bg-red-500/50 transition-all"
                                title="Hapus"
                            >
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex items-center justify-between mt-3">
                            <span class="inline-block px-2 py-0.5 text-[10px] font-medium text-primary-300 bg-primary-500/10 rounded-md border border-primary-500/20">{{ $item->category }}</span>
    ...
    <template x-teleport="body">
        <div x-show="showDeleteModal"
             ...
             style="display: none;">
            <div class="bg-slate-900 rounded-2xl p-6 max-w-sm w-full mx-4 border border-slate-700/50 shadow-2xl shadow-black/50"
                 x-transition:enter="transition ease-out duration-200"
                ...
                <div class="text-center">
                    ...
                    <template x-if="isBulkDelete">
                        <div>
                            <h3 class="text-lg font-semibold text-white mb-2">Hapus <span x-text="selectedItems.length"></span> Item?</h3>
                            <p class="text-sm text-slate-400 mb-6">Anda akan menghapus <span class="font-semibold text-red-300" x-text="selectedItems.length"></span> item secara permanen. Tindakan ini tidak dapat dibatalkan.</p>
                        </div>
                    </template>
                    <template x-if="!isBulkDelete">
                        <div>
                    ...
                    <div class="flex gap-3">
                        <button @click="closeModal()" class="flex-1 py-2.5 px-4 rounded-xl border border-slate-700 text-slate-300 text-sm font-medium hover:bg-slate-800 transition-all" :disabled="deleting">
                            Batal
                        </button>
                        <button @click="confirmDelete()" class="flex-1 py-2.5 px-4 rounded-xl bg-red-500/80 hover:bg-red-500 text-white text-sm font-medium transition-all disabled:opacity-50" :disabled="deleting">
                            <span x-show="!deleting">
                                <template x-if="isBulkDelete"><span>Hapus <span x-text="selectedItems.length"></span> Item</span></template>
                                <template x-if="!isBulkDelete"><span>Hapus</span></template>
                            </span>
